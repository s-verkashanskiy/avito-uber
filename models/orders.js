...
orderSchema.static("getOrdersWithCategory", async function (categoryName) {
  ...
  const result = orders.filter( order => order.categories.some(category => category.title === categoryName))
  .map(order => { 
    return {
    title: order.title,
    description: order.description,
...
orderSchema.static("getOrdersWithSkill", async function (skillName) {
  ...
  const result = orders.filter( order => order.skills.some(skill => skill.title === skillName))
  .map(order => { 
    return {
    title: order.title,
    description: order.description,
